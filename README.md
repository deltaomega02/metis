...
- **판단 주체는 세대에 따라 바뀌었다.** v1~v3은 *AI = 판단자(방향·강도·진입 자리) / 코드 = 실행자(SL·TP·청산·안전 검증)* 구조였다.
  그러나 페이퍼·실전 병행 검증에서 **AI 판단이 규칙 기반보다 낫다는 근거가 나오지 않았고**, v5에서 AI를 판단 경로에서 제거했다.
  **현행 v6는 규칙 기반이며 LLM을 사용하지 않는다** (`v6/code/README.md` 참고).
