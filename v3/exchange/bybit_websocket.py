...
class BybitWebSocket:
    ...
    def __init__(
        ...
        on_execution_callback: Optional[Callable[[list], None]] = None,
        ...
        Args:
            ...
            on_execution_callback: 체결 이벤트 콜백 (executions: list) — Fix #27
        ...
        try:
            ...
            if data.get("op") == "auth":
                if data.get("success"):
                    ...
                    # position + execution 구독 (Fix #27: execution 추가)
                    subscribe_msg = {
                        "op": "subscribe",
            ...
            if data.get("op") == "subscribe":
                if data.get("success"):
                    logger.info("Position + Execution 구독 성공")
                return
    ...
    def _handle_execution_update(self, executions: list):
        """Fix #27: Bybit V5 Private WS execution 토픽 수신.

        체결 이벤트가 *즉시* push되므로 청산 직후 cache에 도달.
        position_manager._get_server_triggered_execution()가 이 cache 조회 가능.
        """
        if not executions:
            return
        try:
            count = len(executions)
            logger.info(f"Execution WS 이벤트: {count}건 수신")
            if self.on_execution_callback:
                self.on_execution_callback(executions)
        except Exception as e:
            logger.error(f"Execution 처리 오류: {e}")
