...
class GeminiClient:
    ...
    def filter_entry(
        ...
        try:
            ...
            result["next_recheck_hours"] = max(0.25, min(24.0, nrh))
    ...
    def analyze_long(self, market_data: Dict[str, Any], symbol: str) -> Dict[str, Any]:
        ...
        # next_recheck 클램프
        try:
            result["next_recheck_hours"] = max(0.25, min(24.0, float(result.get("next_recheck_hours", 4.0))))
        except (TypeError, ValueError):
            result["next_recheck_hours"] = 4.0
        # if_taken 정규화
