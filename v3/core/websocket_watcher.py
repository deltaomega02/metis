...
class FuturesWatcher:
    ...
    def __init__(
        ...
        # WebSocket (가격 콜백 + 강제청산 콜백 + 체결 콜백 + active symbol)
        # Fix #27: execution_cache로 실시간 체결 push 수신
        ...
        self.ws = BybitWebSocket(
            ...
            on_execution_callback=execution_cache.push,
            symbol=self.symbol
        )
        ...
        # Profit Guard 상태 (Fix #32: peak 추적 추가)
        self.profit_guard_active = False
        self._current_unrealized_pnl_pct = 0.0
        self._session_peak_pnl_pct = 0.0  # 진입 후 최고 PnL% (drawdown 계산용)
    ...
    def _update_profit_guard_flag(self, current_price: float):
        """Profit Guard 활성화 플래그 업데이트 (Fix #32: coin별 임계 + peak 추적)"""
        if self.direction == "LONG":
        ...
        # Peak 추적 (drawdown 계산용)
        if self._current_unrealized_pnl_pct > self._session_peak_pnl_pct:
            self._session_peak_pnl_pct = self._current_unrealized_pnl_pct

        from config import PROFIT_GUARD
        # Coin별 활성화 임계
        is_alt = ("SOL" in self.symbol) or ("ETH" in self.symbol) or ("XRP" in self.symbol)
        act_threshold = PROFIT_GUARD.ALT_ACTIVATION_PCT if is_alt else PROFIT_GUARD.ACTIVATION_PCT

        was_active = self.profit_guard_active
        self.profit_guard_active = self._current_unrealized_pnl_pct >= act_threshold

        if self.profit_guard_active and not was_active:
            logger.info(
                f"Profit Guard v2 활성화 ({self.symbol}): 미실현 수익률 "
                f"{self._current_unrealized_pnl_pct * 100:.2f}% >= "
                f"{act_threshold * 100:.1f}% (coin별 임계)"
            )
