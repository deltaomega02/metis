...
class PositionManager:
    ...
    def _get_server_triggered_execution(
        ...
        # Fix #27: 진입 시각 파싱은 함수 진입 직후 한 번 (cache 조회 + 폴링 모두 사용)
        try:
        ...
        # Fix #27: WebSocket execution cache 우선 조회 (실시간 push)
        # Bybit Private WS execution 토픽이 청산 체결 즉시 push → cache.
        # API 폴링 대비 압도적 빠름.
        from core.execution_cache import execution_cache

        def _try_cache() -> Optional[Tuple[float, float]]:
            cached = execution_cache.get_after(symbol, close_side, entry_time_ms)
            if not cached:
                return None
            total_qty = 0.0
            total_value = 0.0
            total_fee = 0.0
            for ex in cached:
                total_qty += ex["exec_qty"]
                total_value += ex["exec_qty"] * ex["exec_price"]
                total_fee += ex["exec_fee"]
            if total_qty > 0 and total_value > 0:
                avg = total_value / total_qty
                logger.info(
                    f"체결 cache hit (WS): 건수={len(cached)} 총수량={total_qty} "
                    f"평균가={avg:.4f} 총수수료={total_fee:.6f}"
                )
                return avg, total_fee
            return None

        # Fix #33: cache 진단 로그 추가
        cache_size = execution_cache.size()
        logger.info(f"WS cache 상태 (체결 조회 진입 시): {cache_size}")

        # 청산 직후 cache에 *이미 도착*했을 수 있음
        cached_result = _try_cache()
        if cached_result is not None:
            return cached_result

        # 짧은 대기 후 cache 재확인 (1.5s) — WS push가 청산 시점과 거의 동시
        time.sleep(1.5)
        cached_result = _try_cache()
        if cached_result is not None:
            return cached_result

        # Fix #33: REST 폴링 fallback 보강 (35s → 80s)
        # 진단 결과: Bybit API 청산 체결 반영 지연 30s+ 빈번. 35s 부족.
        # 초기 20s + 6회 × 10s = 80s 총 대기.
        logger.warning("WS cache miss — REST 폴링 fallback 진입 (Fix #33: 총 80s 대기)")
        time.sleep(18.0)  # 초기 추가 대기 (1.5s 이미 + 18.5s = 20s 총)

        # 재시도 로직: 6회 × 10s = 60s. cache는 retry 마다 재확인.
        for attempt in range(6):
            if attempt > 0:
                time.sleep(10.0)
                logger.info(f"체결 내역 조회 재시도 ({attempt + 1}/6)")
                cached_result = _try_cache()
                if cached_result is not None:
                    return cached_result

            try:
                # Fix #33: limit 100으로 증가 (Bybit max 90 반환이지만 안전 마진)
                ...
                if not executions:
                    logger.warning(f"체결 내역 없음 (시도 {attempt + 1}/6)")
                    continue
                ...
                if not matched_executions:
                    logger.warning(f"진입 이후 청산 체결 내역 없음 (시도 {attempt + 1}/6)")
                    continue
                ...
                logger.warning(f"유효한 체결 데이터 없음 (시도 {attempt + 1}/6)")
                
            except Exception as e:
                logger.error(f"서버 체결 내역 조회 실패 (시도 {attempt + 1}/6): {e}")
    ...
    def close_position(
        ...
        if exchange_position and exchange_position.get("size", 0) > 0:
            ...
            if actual_exit_price is None:
                ...
                # Fix #34: avg_price=0 시 WS cache 시도 (시장가 청산 직후 push 가능)
                if not actual_exit_price or actual_exit_price == 0:
                    time.sleep(2.0)
                    from core.execution_cache import execution_cache
                    close_side = "Sell" if direction == "LONG" else "Buy"
                    try:
                        entry_ms_for_cache = int(datetime.fromisoformat(entry_timestamp).timestamp() * 1000)
                    except Exception:
                        entry_ms_for_cache = 0
                    cached = execution_cache.get_after(symbol, close_side, entry_ms_for_cache)
                    if cached:
                        total_qty_c = sum(c["exec_qty"] for c in cached)
                        total_value_c = sum(c["exec_qty"] * c["exec_price"] for c in cached)
                        total_fee_c = sum(c["exec_fee"] for c in cached)
                        if total_qty_c > 0:
                            actual_exit_price = total_value_c / total_qty_c
                            exit_fee = total_fee_c
                            logger.info(
                                f"Fix #34 WS cache 폴백 성공: 체결가={actual_exit_price:.4f} "
                                f"수수료={exit_fee:.4f} (건수={len(cached)})"
                            )

                # WS cache 비어있으면 REST 폴링 시도 (server-triggered 경로 재사용)
                if not actual_exit_price or actual_exit_price == 0:
                    logger.warning("Fix #34 시장가 청산 후 WS cache 비어있음 — REST 폴링 시도")
                    try:
                        rest_price, rest_fee = self._get_server_triggered_execution(
                            direction=direction,
                            entry_timestamp=entry_timestamp,
                            symbol=symbol
                        )
                        if rest_price is not None and rest_price > 0:
                            actual_exit_price = rest_price
                            exit_fee = rest_fee
                            logger.info(f"Fix #34 REST 폴링 폴백 성공: 체결가={actual_exit_price:.4f}")
                    except Exception as e:
                        logger.warning(f"Fix #34 REST 폴링 폴백 실패: {e}")

                # 모든 폴백 실패 시 ticker 폴백 (PnL 부정확 위험)
                if not actual_exit_price or actual_exit_price == 0:
                    ticker = self.client.get_ticker(symbol)
                    actual_exit_price = ticker.get("last_price", entry_price)
