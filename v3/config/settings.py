...
class TradingConfig:
    ...
    # 멀티 심볼: BTC + SOL + ETH 동시 감시. 시그널 강한 쪽 진입 (단일 포지션).
    SYMBOLS: Tuple[str, ...] = ("ETHUSDT", "SOLUSDT", "XRPUSDT")
    ...
    def __post_init__(self):
        ...
        symbol_specs = {
            "XRPUSDT": {"min_order_qty": 1, "qty_precision": 0, "price_precision": 4},
            "SOLUSDT": {"min_order_qty": 0.1, "qty_precision": 1, "price_precision": 2},
            "ETHUSDT": {"min_order_qty": 0.01, "qty_precision": 2, "price_precision": 2},
        }
...
class ProfitGuardConfig:
    """Profit Guard v2 (Fix #32) — 3-Trigger 시스템.

    피크 흑자 후퇴 회피 + 큰 흑자 보호.

    - Trigger 1: Multi-TF Reversal (5m + 15m 동시 MACD/RSI 반전 시)
    - Trigger 2: Peak Drawdown (피크 6%+ 도달 후 피크 일정 비율 후퇴 시)
    - Trigger 3: Hard Drawdown (피크 10%+ 도달 후 절대 5pp 후퇴 시)
    """
    # === 활성화 (coin별 차별화) ===
    ACTIVATION_PCT: float = 0.015  # 단타 1.5% (5x × 0.3% 가격)
    ALT_ACTIVATION_PCT: float = 0.012  # 단타 ALT 1.2%

    # === 감시 주기 ===
    CHECK_INTERVAL_SEC: int = 30  # 60→30 (반전 빠르게 감지)

    # === Multi-TF (Trigger 1) ===
    KLINE_INTERVAL: str = "15"  # primary
    KLINE_INTERVAL_SECONDARY: str = "5"  # secondary, 빠른 신호
    KLINE_LIMIT: int = 60
    DUAL_TF_CONFIRMATION: bool = True  # 둘 다 반전 시만 EXIT
    ...
    # RSI 파라미터
    RSI_PERIOD: int = 14
    RSI_REVERSAL_THRESHOLD: float = 3.0  # 5.0→3.0 (더 민감)

    # === Peak Drawdown (Trigger 2) — 핵심 신규 ===
    MIN_PEAK_FOR_DRAWDOWN: float = 0.015  # 단타 1.5%+ 도달 후
    DRAWDOWN_RATIO_BTC: float = 0.30  # 단타 30% 후퇴 (빠른 lock)
    DRAWDOWN_RATIO_ALT: float = 0.30  # 단타 ALT 30%

    # === Hard Drawdown (Trigger 3) — 큰 흑자 보호 ===
    HARD_DRAWDOWN_MIN_PEAK: float = 0.03  # 단타 3%+ 도달 후
    HARD_DRAWDOWN_ABS_PP: float = 0.015  # 단타 1.5pp 절대 후퇴
